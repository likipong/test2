...
1. 開くと起動シーケンスのあと、サンプル線区のダイヤが自動で生成されます（278 本・17 編成・40 行路）。
2. **ダイヤグラム** … 掃引しながらスジが引かれます。スジをクリックすると、その列車が属する運用の 27 本が白い芯で浮かび上がります。
3. **運行モニタ** … 「運転開始」を押すと模擬時刻が 60 倍速で進み、在線表示盤の上を列車の光点が動きます。ダイヤグラムに切り替えると、現在時刻の縦線と在線列車の光点が一緒に動きます。
4. **遅延を注入** … いま走っている列車の 1 本に 5 分の遅れを与えます。その列車と波及先が橙に変わり、駅の発車案内の時刻がずれ、ダイヤグラムには遅延ダイヤが破線で重なります。「復旧」で計画ダイヤに戻ります。
